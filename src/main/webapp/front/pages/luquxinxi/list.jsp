			...
			<div class="recommend" :style='{"margin":"10px auto","flexWrap":"wrap","background":"none","display":"flex","width":"90%","position":"relative","justifyContent":"space-between"}'>
				...
				<form class="layui-form filter" :style='{"padding":"10px 10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					...
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						...
						<input type="text" :style='{"padding":"0 10px","margin":"0","outline":"none","borderColor":"#98c6e2 #5fa6d0 #98c6e2 #5fa6d0","color":"#333","borderRadius":"4px","borderWidth":"8px","width":"140px","fontSize":"14px","lineHeight":"40px","borderStyle":"solid","height":"48px"}' name="biaoti" id="biaoti" placeholder="标题" autocomplete="off" class="layui-input">
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>专业名称</div>
						<select name="zhuanyemingcheng" id="zhuanyemingcheng" lay-filter="zhuanyemingcheng">
							<option value="">请选择</option>
							<option v-for="(item,index) in zhuanyemingchengOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>录取分数</div>
						<select name="luqufenshu" id="luqufenshu" lay-filter="luqufenshu">
							<option value="">请选择</option>
							<option v-for="(item,index) in luqufenshuOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
					...
					<button :style='{"cursor":"pointer","padding":"0px 10px","margin":"0 4px 0 0","borderColor":"#ffc766","color":"#333","outline":"none","borderRadius":"4px","background":"#fff","borderWidth":"8px","width":"auto","fontSize":"14px","lineHeight":"36px","borderStyle":"outset","height":"48px"}' v-if="isAuth('luquxinxi','新增')" @click="jump('../luquxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
				...
				<div :style='{"width":"100%","margin":"20px 0 10px","minWidth":"1050px","background":"none","flex":"1","order":"3"}' class="lists">
					...
					<div :style='{"padding":"0px","margin":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}' class="list list-1">
						<div :style='{"border":"1px solid #e6e6e6","padding":"10px","margin":"0 0 20px","flexWrap":"wrap","background":"rgba(255,255,255,.8)","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' @click="jump('../luquxinxi/detail.jsp?id='+item.id)" v-for="(item,index) in dataList" :key="index" class="list-item animation-box">
							<img :style='{"cursor":"pointer","border":"0px inset #98c6e2","padding":"0px","boxShadow":"0px 56px 36px -60px #999","objectFit":"cover","borderRadius":"4px","display":"block","width":"100%","height":"275px"}' :src="item.fengmian?baseurl+item.fengmian.split(',')[0]:''">
							...
							<div :style='{"cursor":"pointer","padding":"4px 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#3d74c0","textAlign":"center","display":"inline-block","width":"100%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name">{{item.biaoti}}</div>
							<div :style='{"cursor":"pointer","padding":"4px 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#3d74c0","textAlign":"center","display":"inline-block","width":"100%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name">{{item.gaoxiaodizhi}}</div>
						</div>
					</div>
			...
			layui.use(['form', 'layer', 'element', 'carousel', 'laypage', 'http', 'jquery','laydate', 'slider'], function() {
				...
				http.request('config/list', 'get', {
        			...
        			http.request(`option/zhuanyemingcheng/zhuanyemingcheng`,'get',{},(res)=>{
                			vue.zhuanyemingchengOptions = res.data
							vue.$nextTick(() => {form.render()})
        			});
        			http.request(`option/luqufenshu/luqufenshu`,'get',{},(res)=>{
                			vue.luqufenshuOptions = res.data
							vue.$nextTick(() => {form.render()})
        			});
				...
				function pageList() {
			        ...
			        if (jquery('#luqufenshu').val()) {
			          param['luqufenshu'] = jquery('#luqufenshu').val() ? jquery('#luqufenshu').val() : '';
			        }
