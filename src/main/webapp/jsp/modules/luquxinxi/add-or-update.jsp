...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑录取信息</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>编辑录取信息</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>录取分数</label>
					<select id="luqufenshuSelect" name="luqufenshu" class="form-control">
							<option value="">请选择录取分数</option>
					</select>	
				</div>	
				<div class="form-item">
					<label>录取人数</label>
					<input name="luqurenshu" id="luqurenshu" name="luqurenshu" class="form-control" placeholder="请输入录取人数">
				</div>
				<div class="form-item">
					<label>发布时间</label>
				...
				<div class="form-item-file" >
					...
					<div class="upload btn-file">
						插入图片
						<input id="luquneirongupload" name="file" type="file">
					</div>
					<script id="luquneirongEditor" type="text/plain"></script>
					<script type="text/javascript">
						//建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
					   //相见文档配置属于你自己的编译器
					 var luquneirongUe = UE.getEditor('luquneirongEditor', {
						toolbars: [
							[
								'undo', //撤销
								'bold', //加粗
					...
					<input type="hidden" name="luquneirong" id="luquneirong-input">
				</div>
				<div class="form-btn">
	...
	<script>
			...
			http("option/luqufenshu/luqufenshu","GET",{},(res)=>{
				if(res.code == 0){
					...
					for(var i=0;i<luqufenshuOptions.length;i++){
						...
						if(ruleForm.luqufenshu == luqufenshuOptions[i]){
							luqufenshuOption.setAttribute('selected','selected');
						}
		...
		function upload(){
			...
			$('#luquneirongupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('luquneirongEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.luquneirong != null && ruleForm.luquneirong != 'null' && ruleForm.luquneirong != ''){
				var luquneirongMes = '' + ruleForm.luquneirong;
				var luquneirongUeditor = UE.getEditor('luquneirongEditor');
				luquneirongUeditor.ready(function() {
					luquneirongUeditor.setContent(luquneirongMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('luquneirongEditor').hasContents()){
				$('#luquneirong-input').attr('value',UE.getEditor('luquneirongEditor').getContent());
			}
		}
		...
		function dataBind(){
			...
			var luqufenshuOptions = document.getElementsByClassName("luqufenshuOption");
			for(var luqufenshuCount = 0; luqufenshuCount < luqufenshuOptions.length;luqufenshuCount++){
				if(luqufenshuOptions[luqufenshuCount].getAttribute('value') == ruleForm.luqufenshu){
					luqufenshuOptions[luqufenshuCount].setAttribute('selected','selected');
				}
			}
			$("#luqurenshu").val(ruleForm.luqurenshu);	
			$("#luquneirong").val(ruleForm.luquneirong);	
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.luqufenshu != null && res.data.luqufenshu != ''){
						...
						var luqufenshuOptions = document.getElementsByClassName("luqufenshuOption");
						for(var luqufenshuCount = 0; luqufenshuCount < luqufenshuOptions.length;luqufenshuCount++){
							if(luqufenshuOptions[luqufenshuCount].getAttribute('value') == res.data.luqufenshu){
								luqufenshuOptions[luqufenshuCount].setAttribute('selected','selected');
							}
						}
					}
					if(res.data != null && res.data != '' && res.data.luqurenshu != null && res.data.luqurenshu != ''){
						...
						$("#luqurenshu").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.luquneirong != null && res.data.luquneirong != ''){

						$("#luquneirong").val(res.data.luquneirong);
                        var luquneirongMes = '' + res.data.luquneirong;
                        var luquneirongUeditor = UE.getEditor('luquneirongEditor');
                        luquneirongUeditor.ready(function() {
                            luquneirongUeditor.setContent(luquneirongMes);
                        });
